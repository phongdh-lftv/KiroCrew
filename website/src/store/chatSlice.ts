...
>(
  ...
  async (arg, { dispatch, getState, rejectWithValue }) => {
    ...
    // Watermark = the newest message ts this window knows for the slot, so
    // the relay cannot clear a badge lit by a message that landed after
    // this open (server-minted ts when available; client now as fallback).
