...
export default function DevFleetPage() {
  ...
  // They are ordered by the lane order the payload publishes (stable, insider,
  // nightly), which is stable across refreshes.
  ...
  function rowButtons(w: Worktree): ReactNode[] {
    ...
    out.push(<MenuBtn key="menu" items={[
      ...
      channelFor(w)
        ? { label: i18nT('pages.devFleetPage.advance_to_channel_tip'), icon: <RefreshCw size={13} className="lucide-inline" />, onClick: () => advanceReleaseChannel(channelFor(w)!.lane), disabled: !!rcBusy[channelFor(w)!.lane] || !!channelFor(w)!.at_tip, title: channelFor(w)!.at_tip ? i18nT('pages.devFleetPage.at_the_channel_tip') : i18nT('pages.devFleetPage.moves_the_detached_worktree_to_the_channel_tip_refused_if_dirty') }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '16px 84px minmax(0,1fr) 64px 48px 44px 212px', gap: 8, alignItems: 'center', padding: '5px 0', borderTop: '1px solid var(--border)', minHeight: 30, minWidth: 640 } as CSSProperties}>
          ...
          <div style={{ minWidth: 0, display: 'flex', alignItems: 'baseline', gap: 6, whiteSpace: 'nowrap', overflow: 'hidden' } as CSSProperties}>
            ...
            {/* Release-channel pin. The lane is already in the row NAME
                ...
                name cannot: which release the tree is actually sitting on.
                A `mismatch` lane_check means the tag-shape rule and the version
                classifier disagreed, which is the one way this row could be
                serving a prerelease while claiming a stable lane — so it is
                shown as a warning rather than folded into the normal case. */}
            {channelFor(w) ? (
              <Badge
                variant={channelFor(w)!.lane_check === 'mismatch' ? 'err' : channelFor(w)!.at_tip ? 'ok' : 'warn'}
                className="text-[10px] px-1.5 py-0"
                title={channelFor(w)!.lane_check === 'mismatch'
                  ? i18nT('pages.devFleetPage.release_channel_lane_mismatch', { lane: channelFor(w)!.lane, ref: channelFor(w)!.ref || '?' })
  ...
  // than at the end so the three lanes read as one group.
