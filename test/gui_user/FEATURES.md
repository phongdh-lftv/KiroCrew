...
`docs/system-specs/modules/**` at commit `119686df35fb`, then deduped by route + flow. It is an
...
| Browser panel (`browser-panel`) | 4 | 0 | 2 | 1 | 0 | 1 |
| Voice (`voice`) | 6 | 0 | 3 | 0 | 2 | 1 |
| Notifications (`notifications`) | 7 | 2 | 2 | 1 | 0 | 2 |
| Onboarding & login (`onboarding`) | 6 | 1 | 5 | 0 | 0 | 0 |
| Search everywhere & command palette (`search`) | 3 | 2 | 1 | 0 | 0 | 0 |
| Developer tools (`developer`) | 16 | 1 | 14 | 0 | 0 | 1 |

Priorities: P0 12 · P1 36 · P2 167 · P3 63. Deduped from 387 raw records.
...
| P2 | `settings-browser` | As a user, I want to install playwright-cli, set an attach token and choose the engine, so that the agent can browse. | `/settings/browser` | minimal | nightly | 3 |

## Voice (`voice`)

| Priority | Id | User story | Start URL | Seed | Runnable | Steps |
|---|---|---|---|---|---|---|
| P2 | `settings-voice` | As a user, I want to pick a TTS provider and voice and toggle Auto-speak, so that spoken interaction sounds right and the choice survives a reload. | `/settings/voice` | minimal | nightly | 4 |
| P2 | `settings-voice-stt` | As a user, I want to configure the dictation engine, model and hotkey and see which providers support live streaming, so that I pick one that works on my host. | `/settings/voice` | minimal | nightly | 3 |
...
| P2 | `settings-notifications` | As a user, I want to choose which events notify, on which channel, and with which sound, so that I get signal, not noise. | `/settings/notifications` | minimal | nightly | 3 |

## Onboarding & login (`onboarding`)

| Priority | Id | User story | Start URL | Seed | Runnable | Steps |
|---|---|---|---|---|---|---|
| P0 | `standalone-kiro-sign-in` | As a user, I want to sign in to the Kiro identity agents run as (Google, GitHub, Builder ID, SSO) and see an explicit expired state, so that turns never silently fail on auth. | `/settings/overview` | minimal | smoke | 4 |
...
| P2 | `settings-imports` | As a switcher, I want to import config and history from another tool, so that I start with my existing setup. | `/settings/imports` | onboarding-fresh | nightly | 3 |
...
| P0 | `search-settings-search` | As a user, I want to search settings, so that I can find a specific toggle without browsing tabs. | `/settings` | minimal | smoke | 3 |
...
| Priority | Id | User story | Start URL | Seed | Runnable | Steps |
|---|---|---|---|---|---|---|
| P1 | `settings-developer` | As a power user, I want the Developer Mode gate, Feature Previews opt-ins and the local-gateway switch, so that I can turn on unreleased surfaces. | `/settings/developer` | minimal | smoke | 3 |
...
| needs-secret | `settings-overview-kiro-sign-in-flow` | settings | Sign in / sign out of the Kiro account from Overview | Embedded KAS login flow needs a real external identity; cannot be completed against the fake backend. |
...
| needs-secret | `app-meetings-live-transcribe` | apps | Meetings: start a meeting and capture live transcript with notes | Retiered from nightly: the primary flow needs a working STT backend (an optional heavy install the no-network target cannot fetch) and a calendar/audio source; only the empty meeting list and settings views render from … |
...
| needs-secret | `app-pptx-maker-studio` | apps | PPTX Maker: open the studio, browse the library, and start a deck chat | Retiered from nightly: the primary flow (start a deck chat, watch preview) requires the engine download and MCP tools that need network, which the target does not have; the Decks/Library/Settings views render headless b… |
...
| excluded | `chat-remote-bound-session` | chat | Remote-bound session | Retiered from needs-secret to excluded: this is not a missing credential but a missing second live Kiro Crew instance plus a network tunnel, which the Xvfb+fake-ACP, no-network target cannot provide. Only the flag-gated… |
...
| excluded | `kas-auth-chat-error-row-sign-in-link` | chat | An auth-required turn renders a 'Sign in to Kiro' error row deep-linking to the card | P3 until the harness gap closes: a P0 surface (newly merged KasLoginGate sign-in card, reached from this error row) but currently excluded: the fake ACP backend cannot emit the AcpAuthRequired vocabulary (only [[ERROR]]… |
...
| excluded | `app-dev-fleet-pods` | apps | Dev Fleet: list worktrees and bring one up as a pod | Primary actions (pull+rebuild, rebase, prune, provision pods) are destructive to the host's real checkout and require systemd; the read-only fleet list could be a degraded nightly check. |
...
| excluded | `knowledge-fetch-url-source` | knowledge | Add a URL source that is fetched and ingested | Retiered from needs-secret: no external account is involved, the blocker is outbound network, which the target does not have, so the fetch fails by construction and the flow is unobservable here. |
...
| excluded | `chat-voice-reply-and-dictation` | voice | Voice reply and dictation | Retiered from native-only to excluded: the Electron shell would not add a microphone, an audio sink or a recogniser model to the Xvfb host, so no native lane on this target could observe the behaviour either. The mic bu… |
...
existing slug for now. Adding a slug is a one-line change to the registry plus a row in
