...
def pod(tmp_path_factory: pytest.TempPathFactory):
    ...
    try:
        ...
        if client is not None:
            # One composite verdict rather than a chain: every leg below would be
            # trivially true whenever `pod_is_gone` holds, and an assertion that
            # cannot fail is decoration.
            assert pod_is_gone, (
                f"`pod down {name}` did not confirm the pod is gone, so NOTHING was "
                f"deleted -- a live pod's state must not be removed from under it. "
                ...
                f"Left in place for inspection and manual cleanup: plane={scratch}, "
                f"home={client.home}"
            )
