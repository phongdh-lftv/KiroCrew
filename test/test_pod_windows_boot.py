...
def plane(tmp_path_factory) -> dict[str, str]:
    ...
    try:
        ...
        if leftover:
            # Say it and leave everything: a task still registered means a process
            # may still be serving out of this tree, and the operator needs both the
            # task name and the path to clean up by hand.
            raise AssertionError(
                "the pod plane was left behind on purpose: Task Scheduler still "
                f"reports {leftover} under the prefix {prefix!r}, so a live pod may "
                f"be serving out of {root}. Nothing was deleted. Remove the task, "
                "then the directory."
            )
