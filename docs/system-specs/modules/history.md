...
- `delete_session(key)` — permanently removes a session JSONL file. Dashboard
  deletion treats every other row from `list_sessions()` as a surviving owner:
  closed transcripts still protect their exact and filename-folded chat-pin,
  work-ledger, and autocompact-override identities. The session-side cleanup
  contract and its fail-safe behavior are specified in [session.md](session.md)
  under **Permanent history deletion keeps sidecar ownership exact**.
