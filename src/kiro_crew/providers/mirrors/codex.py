...
from kiro_crew.acp.session_mcp import (
    ...
    session_mcp_restricted_servers,
)
...
def codex_withheld_servers(
    agent: str | None,
    *,
    work_dir: object = None,
    restricted: frozenset[str] | None = None,
) -> frozenset[str]:
    """Every server name this transport must not mount for *agent*.
    ...
    * it is one of Crew's own identity-bound servers, which cannot be handed this
    ...
    ``restricted`` lets a caller that has ALREADY resolved the first half hand it in.
    The projection must: it derives the translation and this set from ONE parse, so
    that a spec gaining ``disabledTools`` between two reads cannot yield a withhold
    set from the old bytes applied to a translation of the new ones. A caller that
    only needs the union -- the pooled-broker append, which never translates --
    omits it and this resolves it.

    Blocking when it resolves the first half itself, so callers run it off the loop.
    """
    if restricted is None:
        restricted = session_mcp_restricted_servers(agent, work_dir=work_dir)  # type: ignore[arg-type]
...
class CodexProjection(NamedTuple):
    """What one codex ``session/new`` needs from the agent spec, from ONE parse.

    ``params`` is the wire face -- exactly what :meth:`CodexMirror.session_params`
    returns. ``denied_tools`` is NOT wire data and deliberately does not ride inside
    it: the params dict is sent to the adapter verbatim, and a Crew-private key on it
    is one adapter release away from a strict-schema rejection of the whole request.
    It is the set of ``(server, tool)`` pairs -- server spelled as codex registers it
    -- that this session's spec switched off and that the client must refuse when
    codex asks permission for them (:meth:`AcpClient._deny_spec_disabled_tool`).
    """
...
def codex_projection(
    ...
    *,
    ...
    to add and no name the fold could collide with.
    ...
    withheld = codex_withheld_servers(agent, work_dir=work_dir, restricted=projection.restricted)
    ...
    return CodexProjection(params={"mcpServers": out}, denied_tools=denied)
...
class CodexMirror(AgentConfigMirror):
    ...
    def rulings(self) -> Mapping[Concern, Ruling]:
        return {
            ...
            Concern.DENIED_TOOLS: Ruling(
                ...
                "permission for it (AcpClient._deny_spec_disabled_tool, on the "
                "(server, tool) pairs codex_projection derives from the same parse). "
    ...
    def session_params(
        ...
        **kwargs: object,
        ...
        (H13). The client calls :func:`codex_projection` directly rather than this
        method, because it also needs the deny set that function derives on the
        same parse and the wire dict must not carry it; this method IS that
        function's ``params``, pinned equal by a test, so the two cannot drift.
        """
