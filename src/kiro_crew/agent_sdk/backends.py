...
# session results carry ``configOptions`` and never ``modes``, on every frame
# captured off its wire, so for a member a response that is not an error IS the
# successful load. Without membership a reopened session would load, fail the
# ``modes`` check, fall through to ``session/new`` and discard the conversation
# the harness had just restored. Opt-in on captured evidence, like every set here.
